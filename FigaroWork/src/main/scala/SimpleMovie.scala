...
object SimpleMovie {
  ...
  private class Movie {
    val quality = Select(0.3 -> 'low, 0.5 -> 'medium, 0.2 -> 'high)  // distbn of the quality of the movie
  }

  private class Appearance(val actor: Actor, val movie: Movie) {
    def probAward(quality: Symbol, famous: Boolean) =
      // pattern matching similar to OCaml
      (quality, famous) match {           // prob. of award, based on the quality of movie and fame of the actor
    ...
    val award = SwitchingFlip(Apply(movie.quality, actor.famous, (q: Symbol, f: Boolean) => probAward(q, f))) // flips true to false and false to true, everytime proposed
  }
  ...
  private val allAwards: Element[List[Boolean]] = Inject(appearances.map(_.award): _*) // sequence of elements into an element over sequences
